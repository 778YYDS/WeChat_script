...
async function main() {
  ...
  try {
    ...
    const isSuccess = (result.status === 200 || json?.data?.name) && !json?.error;
    ...
    if (isSuccess) {
      ...
      $.msg($.name, \"❌ 加入失败\", `状态码: ${result.status || \"未知\"}`);
      $.log(`❌ 加入失败，状态码: ${result.status || \"未知\"}`);
...
function Env(name) {
  ...
  this.getdata = (key) => typeof $prefs !== \"undefined\" ? $prefs.valueForKey(key)
    : typeof $persistentStore !== \"undefined\" ? $persistentStore.read(key) : null;
  this.setdata = (val, key) => typeof $prefs !== \"undefined\" ? $prefs.setValueForKey(val, key)
    : typeof $persistentStore !== \"undefined\" ? $persistentStore.write(val, key) : false;
  ...
  this.post = (opts, cb) => {
    if (typeof $task !== \"undefined\") return $task.fetch({ ...opts, method: \"POST\" }).then((r) => cb(null, r, r.body), (e) => cb(e));
    if (typeof $httpClient !== \"undefined\") return $httpClient.post(opts, (e, r, d) => cb(e, r, d));
  };
  this.toObj = (str) => { try { return JSON.parse(str); } catch { return null; } };
